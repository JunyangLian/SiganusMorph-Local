...
MM_PER_PIXEL = 0.1
OUT = PROJECT_ROOT / "results" / "model_eval" / "v0.6.6_body_depth_outer_boundary_replacement"
...
def body_depth_row(
    ...
    return {
        ...
        "body_depth_source": body_depth.get("body_depth_source", "body_midline_normal_outer_boundary_max_width"),
...
def draw_visual(
    ...
    line(body_depth.get("dorsal_body_depth_contour", []) or body_result.get("dorsal_body_contour", []), (80, 210, 255, 150), 2)
    line(body_depth.get("ventral_body_depth_contour", []) or body_result.get("ventral_body_contour", []), (80, 210, 255, 150), 2)
    ...
    lines = [
        f"{record.get('image_name')} v0.6.6 outer-boundary body depth",
    ...
    display.save(VIS_OUT / f"{Path(str(record['image_name'])).stem}_v066_outer_boundary_compare.png")
...
def write_readme(
    ...
    lines = ["# v0.6.6 Body-Depth Outer-Boundary Replacement Evaluation", ""]
    ...
    lines.append("")
    lines.append("The old inward `body_core_mask` P8/P9 method has been replaced in-place. The current `P8_geometric`, `P9_geometric`, and `body_depth_geometric_mm` fields are produced by scanning local normals from `body_midline_axis` to the full fish-mask outer boundary in the body-depth search range, with width-profile smoothing and QC.")
    lines.append("")
    lines.append("Old v0.6.5 geometric reference medians: P8 = 8.756 mm, P9 = 7.760 mm, body_depth = 6.212 mm.")
    lines.append("")
    ...
    old_improved = bool((body_comparison["geometric_median_error_mm"] < [8.756, 7.760, 6.212]).all()) if len(body_comparison) >= 3 else False
    lines.append(f"1. New outer-boundary geometric P8/P9 improves over old inward geometric method: {'yes' if old_improved else 'mixed/no'}.")
    lines.append(f"2. New geometric P8/P9 is better than v06 for at least one point: {'yes' if geom_better else 'no'}.")
    lines.append("3. Fin/noise suppression is handled by width-profile smoothing and QC; failed QC rows should be reviewed in visual_comparisons/.")
    lines.append(
        f"4. Geometric P8/P9 default: {'candidate for future default after review' if geom_better else 'no'}; UI role now: Advanced reference / QC suggestion."
    )
    lines.append("")
    ...
    for name in [
        ...
        "body_depth_width_profile_examples.csv",
        "visual_comparisons/",
    ]:
        lines.append(f"- `{name}`")
...
def main() -> None:
    ...
    manifest.to_csv(OUT / "v066_evaluation_manifest.csv", index=False, encoding="utf-8-sig")
    ...
    for idx, record in enumerate(manifest.to_dict("records"), start=1):
        ...
        width_profile_rows.append(
            {
                "image_name": image_name,
                "specimen_id": record.get("specimen_id", ""),
                "subset_source": record.get("subset_source", ""),
                "body_depth_source": body_depth.get("body_depth_source", ""),
                "body_depth_width_profile": json.dumps(body_depth.get("body_depth_width_profile", []), ensure_ascii=False),
                "body_depth_width_profile_smooth": json.dumps(body_depth.get("body_depth_width_profile_smooth", []), ensure_ascii=False),
                "body_depth_selected_section": json.dumps(body_depth.get("body_depth_selected_section", {}), ensure_ascii=False),
                "fin_suppression_regions": json.dumps(body_depth.get("fin_suppression_regions", []), ensure_ascii=False),
            }
        )
        draw_visual(record, gt, current, body_result, body_depth, p6_qc, p6_fallback)
        print(f"[{idx}/{len(manifest)}] v0.6.6 outer-boundary body depth: {image_name}")
    ...
    pd.DataFrame(width_profile_rows).to_csv(OUT / "body_depth_width_profile_examples.csv", index=False, encoding="utf-8-sig")
    write_readme(body_comparison, body_by_image, len(manifest))
    ...
    depth_row = body_comparison[body_comparison["keypoint_name"] == "body_depth_mm"].iloc[0]
    print("Finished v0.6.6 body-depth outer-boundary replacement.")
    print("Old geometric method replaced: yes")
    print("New body_depth_source: body_midline_normal_outer_boundary_max_width")
    ...
    print(f"body_depth v06/geometric median: {float(depth_row['v06_median_error_mm']):.3f} / {float(depth_row['geometric_median_error_mm']):.3f}")
    print(f"QC failed cases: {int((body_qc['body_depth_geometric_qc_pass'] == False).sum())}")
    print("Improved over old geometric: see README.md")
    print(f"Recommended default change: {'yes' if bool(body_comparison['recommend_use_geometric'].all()) else 'no'}")
    print("Recommended UI role: Advanced reference / QC suggestion")
